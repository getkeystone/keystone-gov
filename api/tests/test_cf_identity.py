...
def test_both_admin_emails_load():
    """Both arnaldosepulveda@lrfd.ca and arnaldo@getkeystone.ai must be present."""
    path = _yaml_file(DUAL_ADMIN_YAML)
    result = load_role_config(path)

    assert "arnaldosepulveda@lrfd.ca" in result
    e1 = result["arnaldosepulveda@lrfd.ca"]
...
def test_getkeystone_email_not_treated_as_duplicate_of_lrfd():
    """arnaldo@getkeystone.ai and arnaldosepulveda@lrfd.ca are distinct emails."""
    path = _yaml_file(DUAL_ADMIN_YAML)
    result = load_role_config(path)
